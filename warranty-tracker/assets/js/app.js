...
function render() {
  ...
  $('valueTotal').textContent = money(now.reduce((sum, r) => sum + Number(r.itemCost ?? r.cost ?? 0), 0));
  ...
  const sort = $('sort').value; rows.sort((a,b) => sort === 'endDesc' ? b.warrantyEndDate.localeCompare(a.warrantyEndDate) : sort === 'product' ? a.product.localeCompare(b.product) : sort === 'costDesc' ? Number(b.itemCost ?? b.cost ?? 0)-Number(a.itemCost ?? a.cost ?? 0) : a.warrantyEndDate.localeCompare(b.warrantyEndDate));
