...
function findUserIndexByEmail(array $users, string $email): ?int {
    ...
    foreach($users as $i=>$u) if(normalizeEmail((string)($u['email']??''))===$needle) return $i;
    return null;
}
...
function normalizeTicketForRequester(array $ticket): array {
    $ticket['comments']=array_values(array_filter($ticket['comments']??[],static fn(array $c):bool=>($c['visibility']??'public')==='public'));
    return $ticket;
}
function upsertDirectory(array &$directory,string $role,string $displayName,string $email,string $avatar=''): array {
    ...
    foreach($directory as &$e){
        if(($e['role']??'')===$role&&normalizeEmail((string)($e['email']??''))===$email){
            $e['name']=$displayName; $e['displayName']=$displayName; if($avatar!=='')$e['avatar']=$avatar; $e['active']=true; $e['updatedAt']=gmdate('c'); return $e;
        }
    }
    unset($e);
    $entry=['id'=>bin2hex(random_bytes(8)),'role'=>$role,'name'=>$displayName,'displayName'=>$displayName,'email'=>$email,'avatar'=>$avatar,'active'=>true,'createdAt'=>gmdate('c'),'updatedAt'=>gmdate('c')];
    $directory[]=$entry; return $entry;
}
function saveAvatarData(string $userId,string $dataUrl): string {
    ...
    $raw=base64_decode($m[2],true); if($raw===false) jsonResponse(['error'=>'Invalid avatar data.'],422);
    if(strlen($raw)>2*1024*1024) jsonResponse(['error'=>'Avatar must be 2 MB or smaller.'],422);
    if(!is_dir(AVATAR_DIR)&&!mkdir(AVATAR_DIR,0775,true)&&!is_dir(AVATAR_DIR)) jsonResponse(['error'=>'Could not create avatar folder.'],500);
    $ext=$m[1]==='jpeg'?'jpg':$m[1]; $filename=preg_replace('/[^a-zA-Z0-9_-]/','',$userId).'.'.$ext;
    foreach(glob(AVATAR_DIR.'/'.preg_replace('/[^a-zA-Z0-9_-]/','',$userId).'.*')?:[] as $old) @unlink($old);
...
if(isset($_GET['api'])){
    ...
    if($action==='session'&&$method==='GET'){
        $users=readUsers();
        jsonResponse(['user'=>currentUser(),'agentBootstrapAvailable'=>!hasAgent($users),'testMode'=>TEST_MODE,'project'=>readProject()]);
    }
    ...
    if($action==='login'&&$method==='POST'){
        ...
        $users=readUsers(); $user=findUserByEmail($users,$email);
        $password=(string)($payload['password']??'');
        $passwordOk=TEST_MODE || (($user['passwordHash']??'')!=='' && password_verify($password,(string)$user['passwordHash']));
    ...
    if($action==='logout'&&$method==='POST'){
        ...
        session_destroy(); jsonResponse(['ok'=>true]);
    }
    ...
    if(($action==='register-requester'||$action==='bootstrap-agent')&&$method==='POST'){
        ...
        $users[]=$user; $directory=readDirectory(); upsertDirectory($directory,$role,$displayName,$email,$avatar);
        ...
        $_SESSION['ticketing_user']=publicUser($user); session_regenerate_id(true); jsonResponse(['user'=>$_SESSION['ticketing_user']],201);
    }

    if($action==='profile'){
        $sessionUser=requireUser();
        if($method==='GET') jsonResponse(['user'=>$sessionUser]);
        if($method==='PUT'){
            $users=readUsers(); $index=findUserIndexByEmail($users,(string)$sessionUser['email']);
            if($index===null) jsonResponse(['error'=>'User account not found.'],404);
            $displayName=trim((string)($payload['displayName']??'')); if($displayName==='') jsonResponse(['error'=>'Display name is required.'],422);
            $users[$index]['displayName']=$displayName; $users[$index]['name']=$displayName;
            $avatarData=(string)($payload['avatarData']??'');
            if($avatarData!=='') $users[$index]['avatar']=saveAvatarData((string)$users[$index]['id'],$avatarData);
            $users[$index]['updatedAt']=gmdate('c');
            $directory=readDirectory(); upsertDirectory($directory,(string)$users[$index]['role'],$displayName,(string)$users[$index]['email'],(string)($users[$index]['avatar']??''));
            if(!writeJsonFile(USERS_FILE,$users)||!writeJsonFile(DIRECTORY_FILE,$directory)) jsonResponse(['error'=>'Could not save profile.'],500);
            $_SESSION['ticketing_user']=publicUser($users[$index]); jsonResponse(['user'=>$_SESSION['ticketing_user']]);
        }
    }

    if($action==='directory'){
        if($method==='GET'){
            $user=requireUser(); $directory=readDirectory();
            if(($user['role']??'')!=='agent') $directory=array_values(array_filter($directory,static fn(array $e):bool=>normalizeEmail((string)($e['email']??''))===normalizeEmail((string)$user['email'])));
    ...
    if($method==='GET'){
        if(($user['role']??'')==='requester'){
            ...
            $mine=array_values(array_filter($tickets,static fn(array $t):bool=>normalizeEmail((string)($t['email']??''))===$email));
    ...
    if($method==='POST'){
        $subject=trim((string)($payload['subject']??'')); $description=trim((string)($payload['description']??''));
        if($subject===''||$description==='') jsonResponse(['error'=>'Subject and description are required.'],422);
        if(($user['role']??'')==='requester'){$requester=(string)($user['displayName']??$user['name']??'Requester');$email=(string)$user['email'];$assignedTo='';}
        else{$requester=trim((string)($payload['requester']??''));$email=normalizeEmail((string)($payload['email']??''));$assignedTo=trim((string)($payload['assignedTo']??''));if($requester===''||!filter_var($email,FILTER_VALIDATE_EMAIL))jsonResponse(['error'=>'Requester and valid requester email are required.'],422);}
        $now=gmdate('c');
        $ticket=['id'=>bin2hex(random_bytes(8)),'number'=>nextTicketNumber($tickets),'subject'=>$subject,'description'=>$description,'requester'=>$requester,'email'=>normalizeEmail($email),'status'=>'Open','priority'=>validChoice((string)($payload['priority']??'Medium'),['Low','Medium','High','Urgent'],'Medium'),'category'=>trim((string)($payload['category']??'General'))?:'General','assignedTo'=>$assignedTo,'source'=>trim((string)($payload['source']??'Portal'))?:'Portal','createdAt'=>$now,'updatedAt'=>$now,'comments'=>[]];
        $tickets[]=$ticket; if(!writeJsonFile(DATA_FILE,$tickets))jsonResponse(['error'=>'Could not save ticket data.'],500); jsonResponse(['ticket'=>(($user['role']??'')==='requester'?normalizeTicketForRequester($ticket):$ticket)],201);
    }
    if($method==='PUT'){
        $id=trim((string)($payload['id']??''));$updated=null;
        foreach($tickets as &$ticket){
            if(($ticket['id']??'')!==$id)continue;
            if(($user['role']??'')==='requester'){if(normalizeEmail((string)($ticket['email']??''))!==normalizeEmail((string)$user['email']))jsonResponse(['error'=>'You do not have access to this ticket.'],403);}
            else{foreach(['subject','description','requester','email','category','assignedTo','source'] as $field)if(array_key_exists($field,$payload))$ticket[$field]=trim((string)$payload[$field]);if(array_key_exists('status',$payload))$ticket['status']=validChoice((string)$payload['status'],['Open','Pending','Resolved','Closed'],'Open');if(array_key_exists('priority',$payload))$ticket['priority']=validChoice((string)$payload['priority'],['Low','Medium','High','Urgent'],'Medium');}
            $comment=trim((string)($payload['comment']??''));
            if($comment!==''){$visibility=($user['role']??'')==='requester'?'public':validChoice(strtolower((string)($payload['visibility']??'public')),['public','private'],'public');$ticket['comments'][]=['id'=>bin2hex(random_bytes(6)),'author'=>(string)($user['displayName']??$user['name']??ucfirst((string)$user['role'])),'role'=>$user['role'],'visibility'=>$visibility,'body'=>$comment,'createdAt'=>gmdate('c')];}
            $ticket['updatedAt']=gmdate('c');$updated=$ticket;break;
        }
