...
Created: 2026-06-30
Modified: 2026-06-30
Description: Wrapper entrypoint that loads index.php and injects client-side table rendering, trip-session, GPS signal, PHP trip-store helpers, mobile layout refinements, combined speed/time rows, no-key live map display, no-API trip enhancements, and minute-based adjusted ETA.
*/
...
if (is_string($html)) {
    $html = str_replace(
        ...
        'Rev 1.8.7 &bull; Updated 2026-06-30 &bull; Minute Adjusted ETA',
        $html
    );
    ...
    $html = str_replace(
        ...
        '<section class="card"><p class="small"><strong>Useful non-map features still available:</strong> larger map controls, saved ETA offset presets, trip comparison, and improved GPX/KML exports. GPS update age, trip/session start time, live map, background resume recovery, GPS jump filtering, ETA drift, pause count, minute ETA offset, and GPX/KML export are already included. True road-route distance and traffic-aware ETA require a maps/directions API.</p></section>',
        $html
    );
}
