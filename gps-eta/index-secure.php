...
if (is_string($html)) {
    $html = str_replace(
        ...
        'Rev 1.8.12 &bull; Updated 2026-06-30 &bull; Correct Tracking Labels',
        $html
    );
    ...
    $html = str_replace(
        ...
        '<section class="card"><p class="small"><strong>Useful non-map features still available:</strong> saved ETA offset presets, trip comparison, larger map refinements, and improved GPX/KML exports. Dashboard profiles, corrected tracking labels, clear toolbar states, sticky drive mode, map controls, minute ETA offset, live map, GPS jump filtering, ETA drift, pause count, background resume recovery, and GPX/KML export are already included. True road-route distance and traffic-aware ETA require a maps/directions API.</p></section>',
        $html
    );
}
