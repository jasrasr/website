...
Created: 2026-06-30
Modified: 2026-06-30
Description: Wrapper entrypoint that loads index.php and injects helper scripts for the current mobile layout and collapsed changelog fixes.
*/
...
if (is_string($html)) {
    $html = str_replace(
        ...
        'Rev 1.8.14 &bull; Updated 2026-06-30 &bull; Changelog Spacing Fix',
        $html
    );
    ...
    $html = str_replace(
        ...
        '<section class="card"><p class="small"><strong>Useful non-map features still available:</strong> saved ETA offset presets, trip comparison, larger map refinements, and improved GPX/KML exports. Dashboard profiles, collapsed changelog, corrected labels, clear toolbar states, sticky drive mode, map controls, minute ETA offset, live map, ETA drift, pause count, background resume recovery, and GPX/KML export are already included. True road-route distance and traffic-aware ETA require a maps/directions API.</p></section>',
        $html
    );
}
