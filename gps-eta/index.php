...
Created: 2026-06-30
Modified: 2026-06-30
Description: Mobile-friendly PHP page with browser GPS tracking, speed calculation, tracked distance, distance remaining, trip stats, trip log snapshots, CSV export, and dynamic changelog rendering from CHANGELOG.md.
...
<body>
  <main>
    ...
    <section class="card">
      ...
      <div class="rev">Rev <?= h(APP_REVISION) ?> &bull; Updated <?= h(APP_UPDATED) ?> &bull; PHP + Markdown Changelog</div>
    </section>

    <section class="card"><div class="status"><span id="statusDot" class="dot"></span><span id="statusText">Not tracking yet.</span></div></section>
    ...
    <section class="card">
      <h2 style="margin:0 0 10px">Trip Log</h2>
      <div class="log"><table><thead><tr><th>Time</th><th>Elapsed</th><th>Tracked</th><th>Remaining</th><th>Speed</th><th>Accuracy</th></tr></thead><tbody id="logBody"><tr><td colspan="6" class="small">No log entries yet.</td></tr></tbody></table></div>
      <p class="small">A snapshot is logged at start, every 60 seconds while tracking, and any time you tap Log Snapshot.</p>
    </section>

    <section class="card"><p class="small">Accuracy note: phone GPS can be jumpy indoors or at low speeds. This app smooths recent speed readings and ignores obvious GPS jumps. If iOS pauses Safari in the background, tracking may pause too, because apparently the phone has opinions.</p></section>
  </main>
...
Created: 2026-06-30
Modified: 2026-06-30
Description: Uses browser geolocation to calculate smoothed speed, GPS breadcrumb distance traveled, distance remaining, ETA, trip stats, snapshot logs, CSV export, and localStorage trip recovery.
...
function updateStats(){const current=avgCurrentSpeed(),avg=elapsedSeconds>0?distanceTraveledMeters/elapsedSeconds:0;els.elapsedValue.textContent=elapsedSeconds?fmtDur(elapsedSeconds):'--';els.movingValue.textContent=movingSeconds?fmtDur(movingSeconds):'--';els.speedUnit.textContent=speedLabel();els.avgSpeedUnit.textContent=speedLabel();els.maxSpeedUnit.textContent=speedLabel();els.speedValue.textContent=current>=.05?toDisplaySpeed(current).toFixed(1):'--';els.avgSpeedValue.textContent=avg>=.05?toDisplaySpeed(avg).toFixed(1):'--';els.maxSpeedValue.textContent=maxSpeedMps>=.05?toDisplaySpeed(maxSpeedMps).toFixed(1):'--';updateDistanceDisplays();updateEta(current)}
...
function addLog(reason='auto'){const now=new Date(),cur=avgCurrentSpeed(),rem=remainingMeters();if(!tripActive&&reason!=='manual')return;const entry={time:now.toLocaleTimeString([],{hour:'numeric',minute:'2-digit',second:'2-digit'}),iso:now.toISOString(),elapsed:fmtDur(elapsedSeconds),elapsedSeconds,tracked:toDisplayDistance(distanceTraveledMeters),remaining:toDisplayDistance(rem),speed:toDisplaySpeed(cur),accuracy:els.accuracyValue.textContent+' '+els.accuracyUnit.textContent,latlon:els.locationValue.textContent,reason};logEntries.unshift(entry);if(logEntries.length>250)logEntries.pop();renderLog();saveState();lastLogMs=Date.now()}
function renderLog(){if(!logEntries.length){els.logBody.innerHTML='<tr><td colspan="6" class="small">No log entries yet.</td></tr>';return}els.logBody.innerHTML=logEntries.slice(0,25).map(e=>`<tr><td>${e.time}</td><td>${e.elapsed}</td><td>${e.tracked.toFixed(2)} ${label()}</td><td>${e.remaining.toFixed(2)} ${label()}</td><td>${e.speed.toFixed(1)} ${speedLabel()}</td><td>${e.accuracy}</td></tr>`).join('')}
function saveState(){const state={unit:els.unit.value,distanceInput:els.distance.value,tripActive,tripStartMeters,distanceTraveledMeters,elapsedSeconds,movingSeconds,maxSpeedMps,logEntries};localStorage.setItem(STORE_KEY,JSON.stringify(state))}
function loadState(){try{const raw=localStorage.getItem(STORE_KEY);if(!raw)return;const s=JSON.parse(raw);els.unit.value=s.unit||'miles';els.distance.value=s.distanceInput||'';tripActive=!!s.tripActive;tripStartMeters=s.tripStartMeters||0;distanceTraveledMeters=s.distanceTraveledMeters||0;elapsedSeconds=s.elapsedSeconds||0;movingSeconds=s.movingSeconds||0;maxSpeedMps=s.maxSpeedMps||0;logEntries=Array.isArray(s.logEntries)?s.logEntries:[];if(tripActive){els.distance.disabled=true;els.unit.disabled=true;els.startBtn.textContent='Resume Tracking';els.snapshotBtn.disabled=false;els.exportBtn.disabled=false;setStatus('Trip restored. Tap Resume Tracking to continue GPS updates.','warn')}renderLog();updateStats()}catch{localStorage.removeItem(STORE_KEY)}}
...
function reset(){if(watchId!==null)navigator.geolocation.clearWatch(watchId);watchId=null;lastPosition=null;lastTripPosition=null;speedSamplesMps=[];tripActive=false;tracking=false;tripStartMeters=0;distanceTraveledMeters=0;elapsedSeconds=0;movingSeconds=0;maxSpeedMps=0;lastLogMs=0;logEntries=[];els.distance.disabled=false;els.unit.disabled=false;els.startBtn.disabled=false;els.startBtn.textContent='Start Trip Tracking';els.pauseBtn.disabled=true;els.snapshotBtn.disabled=true;els.exportBtn.disabled=true;els.locationValue.textContent='--';els.timestampValue.textContent='--';els.rawGpsValue.textContent='Altitude: -- | Heading: -- | Native speed: --';localStorage.removeItem(STORE_KEY);renderLog();updateStats();setStatus('Trip reset.','warn')}
function exportCsv(){if(!logEntries.length)return;const header=['iso','time','elapsed_seconds','elapsed','tracked_'+label(),'remaining_'+label(),'speed_'+speedLabel(),'accuracy','latlon','reason'];const rows=logEntries.slice().reverse().map(e=>[e.iso,e.time,e.elapsedSeconds,e.elapsed,e.tracked.toFixed(4),e.remaining.toFixed(4),e.speed.toFixed(2),e.accuracy,e.latlon,e.reason]);const csv=[header,...rows].map(r=>r.map(v=>'"'+String(v).replaceAll('"','""')+'"').join(',')).join('\n');const blob=new Blob([csv],{type:'text/csv'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='gps-trip-log.csv';a.click();URL.revokeObjectURL(url)}
