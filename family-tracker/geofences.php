 ...
 * File: geofences.php
 * Revision: 1.5.5
 * Description: Active-group geofence management and browser-driven arrival/departure evaluation.
...
function normalized_zones(array $family): array
{
    ...
    return array_values(array_filter($zones, fn($zone) => is_array($zone) && !empty($zone['id'])));
}
    ...
    foreach ($zones as $zone) {
        ...
        foreach (list_json_records('users') as $member) {
            ...
            if ($previous !== null && $previous !== $inside) {
                ...
                audit_event($inside ? 'geofence_arrival' : 'geofence_departure', ['familyId' => $family['id'], 'userId' => $memberId, 'zoneId' => $zoneId]);
            }
...
function create_zone(array $user, array $family, array $input): void
{
    ...
    if ($name === '' || $latitude === null || $longitude === null || $radius === null) fail('Name, latitude, longitude, and radius are required.', 400);
    ...
    $family['updatedAt'] = now_iso();
    write_family($family);
    add_group_notice((string)$family['id'], 'geofence_created', $user['displayName'] . ' created the place ' . $name . '.', (string)$user['id']);
