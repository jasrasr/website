/* Revision 1.0.0 | 2026-09-17 | Event creation, voting, live rankings and response matrix. */
'use strict';
const $ = id => document.getElementById(id);
const params = new URLSearchParams(location.search);
let eventId = params.get('event'), event = null, editing = false, dirty = false, sequence = 0;
let chosenDates = [], answers = {}, formRevision = 0;
...
function dateLabel(date) { return new Intl.DateTimeFormat(undefined, {weekday: 'short', month: 'short', day: 'numeric', year: 'numeric'}).format(new Date(`${date}T12:00:00`)); }
function link(kind) {
...
$('add-date').onclick = () => {
  ...
  if (!chosenDates.includes(input.value)) chosenDates.push(input.value);
...
$('event-form').onsubmit = async e => {
  ...
  try {
    ...
    applyEvent(data.event); renderLinks();
...
$('edit-event').onclick = () => {
  ...
  for (const key of ['title', 'adminName', 'description']) form.elements[key].value = event[key];
...
function summary() {
  ...
  $('vote-state').textContent = event.closed ? 'This poll is closed. Results remain visible.' : dirty ? 'You have unsaved changes.' : credentials.responseId ? 'Your saved response is loaded.' : 'Choose your dates, then save your response.';
}
function renderVote() {
  ...
  for (const control of $('vote-form').elements) control.disabled = event.closed;
  summary();
}
...
$('vote-form').onsubmit = async e => {
  e.preventDefault(); $('save-vote').disabled = true; ++sequence;
  ...
  try {
    const data = await api({action: 'vote', id: eventId, revision: event.revision, name: $('voter-name').value, answers, responseId: credentials.responseId || '', responseToken: credentials.responseToken || ''});
    Object.assign(credentials, {responseId: data.responseId, responseToken: data.responseToken}); remember(); dirty = false;
    applyEvent(data.event); renderLinks(); message('Your availability is saved. The group results are up to date.');
  } catch (err) { message(err.message, true); }
  finally { renderVote(); }
};
function applyEvent(next) {
  ...
  $('event-meta').textContent = `Organized by ${event.adminName} · ${event.dates.length} dates · ${event.closed ? 'Closed' : 'Open for responses'}`;
  const mine = event.responses.find(response => response.id === credentials.responseId);
  if (!dirty && mine) { answers = {...mine.answers}; $('voter-name').value = mine.name; }
  ...
  if (first || changedDates || !dirty) renderVote();
  renderResults();
}
function renderResults() {
  const total = event.responses.length;
  $('response-count').textContent = `${total} response${total === 1 ? '' : 's'}`;
  const ranked = event.dates.map(date => ({date, yes: event.responses.filter(r => r.answers[date] === 'yes').length, no: event.responses.filter(r => r.answers[date] === 'no').length})).sort((a, b) => b.yes - a.yes || a.date.localeCompare(b.date));
  ...
  $('results').replaceChildren(...ranked.map(row => {
    ...
    item.append(heading, meter, node('p', `${row.yes} can attend · ${row.no} can’t · ${total - row.yes - row.no} unanswered`, 'muted')); return item;
  }));
...
setInterval(() => { if (!editing && !$('save-vote').disabled) refresh(); else if (event?.closed && !editing) refresh(); }, 5000);
